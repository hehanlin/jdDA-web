...
var mychart = require('./mycharts');

$(document).ready(function () {
    var navigating = false, curPage = 1, pages = $('.section').length, $sections = $('.sections'),
        $paginationPage = $('.pagination .page'), $paginationTotal = $('.total-pages'),
        $textStuff = $('.section-heading, .additional-text, .echarts-container');
    if (pages >= 10) {
    ...
    function timeoutNav(t) {
        ...
        setTimeout(function () {
            $('.section-heading, .additional-text, .echarts-container').css({'margin-top': 0 - (parseInt($('.nav-elem.active').attr('data-page')) - 1) * 100 + 'vh'}).hide();
        }, 410);
        setTimeout(function () {
            $textStuff.show();
            $textStuff.css('top');
    ...
    var sidebarScroll = 0, $navEl = $('.nav-elem'), $sidebar = $('.sidebar-real'),
    ...
    $(document).on('click', '.nav-elem:not(.active)', function () {
        ...
        if (Math.abs(activePage - futurePage) > 2) {
            var $fakePage = $('.section-' + futurePage).clone(), $currentPage = $('.section-' + activePage),
                fakeNumber = 0;
